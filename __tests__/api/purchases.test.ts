...
jest.mock('../../lib/sheets', () => ({
  ...
  uploadReceiptImage: jest.fn().mockResolvedValue('fake-url'),
}))

const { appendRows, readRows, updateInventory } = require('../../lib/sheets')
const mockedGetServerSession = getServerSession as jest.Mock

beforeEach(() => {
  jest.clearAllMocks()
  mockedGetServerSession.mockResolvedValue({ accessToken: 'fake-token' })
  // Default: no ingredients in config
  readRows.mockResolvedValue([])
})

test('POST /api/sheets/purchases appends rows for all items and creates missing ingredients', async () => {
  const items = [
  ...
  // Verify missing ingredients are added to config
  expect(appendRows).toHaveBeenCalledWith('fake-token', 'config', [
    ['ingredient', expect.any(String), 'ข้าว', 'Riz', 'kg', '1'],
    ['ingredient', expect.any(String), 'กระเทียม', 'Ail', 'kg', '1'],
  ])
  ...
  expect(updateInventory).toHaveBeenCalledWith('fake-token', [
    { ingredient: 'ข้าว', qtyDelta: 5, unit: 'kg' },
    { ingredient: 'กระเทียม', qtyDelta: 1, unit: 'kg' },
  ])
})
